...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addButton("ApplyMono", &fButton0);
		ui_interface->addHorizontalSlider("Audio", &fHslider0, FAUSTFLOAT(0.5f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.1f));
		ui_interface->addHorizontalSlider("FullAudio", &fHslider1, FAUSTFLOAT(0.5f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.1f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			float fTemp0 = fSlow1 * float(input0[i0]) - fSlow0 * float(input1[i0]);
	...
	FAUST_ADDBUTTON("ApplyMono", fButton0);
	FAUST_ADDHORIZONTALSLIDER("Audio", fHslider0, 0.5f, 0.0f, 1.0f, 0.1f);
	FAUST_ADDHORIZONTALSLIDER("FullAudio", fHslider1, 0.5f, 0.0f, 1.0f, 0.1f);

	#define FAUST_LIST_ACTIVES(p) \
		p(BUTTON, ApplyMono, "ApplyMono", fButton0, 0.0f, 0.0f, 1.0f, 1.0f) \
		p(HORIZONTALSLIDER, Audio, "Audio", fHslider0, 0.5f, 0.0f, 1.0f, 0.1f) \
		p(HORIZONTALSLIDER, FullAudio, "FullAudio", fHslider1, 0.5f, 0.0f, 1.0f, 0.1f) \
...
MyDsp::MyDsp() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void MyDsp::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void MyDsp::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float MyDsp::getParamValue(const std::string& path)
{
